...
namespace DBProject.DAL
{
	...
	public class myDAL
    {
		// Connection string now managed by ConnectionPoolManager with environment variable support
        // Removed hardcoded connection string for cloud deployment
        ...
        {
            // Use connection pool manager for cloud-ready connection handling
            using (SqlConnection con = ConnectionPoolManager.GetConnection())
            {
                try
                {
		...
		public int getTreatmentHistory(int id, ref DataTable result)
		{
            using (SqlConnection con = ConnectionPoolManager.GetConnection())
            {
                try
                {
                    ...
                    DataSet ds = new DataSet();
                    SqlCommand cmd1 = new SqlCommand("RetrieveTreatmentHistory", con);   //Name of your SQL Procedure
                    cmd1.CommandType = CommandType.StoredProcedure;
		...
		public int getNotifications(int pid, ref string dName, ref string timings)
		{
            using (SqlConnection con = ConnectionPoolManager.GetConnection())
            {
                try
                {
                    ...
                    SqlCommand cmd1 = new SqlCommand("RetrievePatientNotifications", con);   //Name of your SQL Procedure
                    cmd1.CommandType = CommandType.StoredProcedure;
		...
		public void GetAllpendingappointments_DAL(int doctorid, ref DataTable DT)
		{
            using (SqlConnection con = ConnectionPoolManager.GetConnection())
            {
                ...
                {
                    System.Diagnostics.Trace.TraceError($"Get all pending appointments error: {ex.Message}");
                }
            }
		}
