...
namespace DBProject.DAL
{
	...
	/// - Uses connection pooling (configured in connection string)
	/// - Implements using statements for proper resource disposal
	/// - Connection string supports environment variable substitution
	/// - Compatible with Cloud SQL Auth Proxy for GCP deployment
	/// - Timeout configurations for cloud resilience
	/// </summary>
	public class myDAL
    {
		/// <summary>
		/// Connection string retrieved from configuration
		/// CLOUD DEPLOYMENT: Configure via environment variables or GCP Secret Manager
		/// Connection string should include: Connection Timeout=30;Max Pool Size=100;Min Pool Size=5;Pooling=true;
		/// For GKE: Use Cloud SQL Auth Proxy sidecar container
		/// </summary>
        private static readonly string connString = GetConnectionString();

		/// <summary>
		/// Gets connection string with support for environment variable override
		...
		private static string GetConnectionString()
		{
			...
			return ConfigurationManager.ConnectionStrings["sqlCon1"].ConnectionString;
		}

		/// <summary>
		/// Creates a new SQL connection with proper timeout and pooling settings
		/// CLOUD READY: Uses connection pooling, proper timeouts
		/// </summary>
		private SqlConnection CreateConnection()
		{
			var connection = new SqlConnection(connString);
			// Connection pooling is handled by the connection string settings
			return connection;
		}
        ...
        {
			// CLOUD READY: Using 'using' statement ensures proper connection disposal and returns to pool
            using (SqlConnection con = CreateConnection())
            {
				...
				try
				{
					...
					cmd1.CommandTimeout = 30; // Cloud-ready timeout
				...
				{
					// TODO: Add structured logging for cloud monitoring
					// Log to Console for container-based logging (GKE)
					Console.WriteLine($"[ERROR] validateLogin failed: {ex.Message}");
					return -1;
				}
				// Connection automatically closed and returned to pool by 'using' statement
			}
        }
        ...
        public int validateUser (string Name, string BirthDate, string Email , string Password , string PhoneNo , string gender , string Address, ref int id)
        {
            ...
            using (SqlConnection con = CreateConnection())
            {
				...
				{
					Console.WriteLine($"[ERROR] validateUser failed: {ex.Message}");
					return -1;
				}
			}
        }
        ...
        public int AddStaff(string Name, string BirthDate, string Phone, char gender, string Address, int salary, string Qual, string Designation)
        {
            ...
            using (SqlConnection con = CreateConnection())
            {
				...
				{
					Console.WriteLine($"[ERROR] AddStaff failed: {ex.Message}");
					return -1;
				}
        ...
        public int DeleteDoctor(int id)
        {
            using (SqlConnection con = CreateConnection())
            {
				...
				{
					Console.WriteLine($"[ERROR] DeleteDoctor failed: {ex.Message}");
					return -1;
				}
        ...
        public int DeleteStaff(int id)
        {
            using (SqlConnection con = CreateConnection())
            {
				...
				{
					Console.WriteLine($"[ERROR] DeleteStaff failed: {ex.Message}");
					return -1;
				}
        ...
        public int GETPATIENT(int pid, ref string name, ref string phone, ref string address, ref string birthDate, ref int age, ref string gender)
        {
            using (SqlConnection con = CreateConnection())
            {
				...
				{
					Console.WriteLine($"[ERROR] GETPATIENT failed: {ex.Message}");
					return -1;
				}
			}
        }
        ...
        public int GET_DOCTOR_PROFILE(int dID, ref string name, ref string phone, ref string gender, ref float charges_Per_Visit, ref float ReputeIndex, ref int PatientsTreated, ref string qualification, ref string specialization, ref int workE, ref int age)
        {
            using (SqlConnection con = CreateConnection())
            {
				...
				{
					Console.WriteLine($"[ERROR] GET_DOCTOR_PROFILE failed: {ex.Message}");
					return -1;
				}
			}
        }
        ...
        public int patientInfoDisplayer(int pid, ref string name, ref string phone, ref string address, ref string birthDate, ref int age, ref string gender)
		{
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] patientInfoDisplayer failed: {ex.Message}");
					return -1;
				}
			}
		}
		...
		public int getBillHistory(int id, ref DataTable result)
		{
			...
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] getBillHistory failed: {ex.Message}");
					return -1;  
				}
			}
		}
		...
		public int appointmentTodayDisplayer(int pid, ref string dName, ref string timings)
		{
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] appointmentTodayDisplayer failed: {ex.Message}");
		...
		public int getTreatmentHistory(int id, ref DataTable result)
		{
			...
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] getTreatmentHistory failed: {ex.Message}");
					return -1;  
				}
			}
		}
		...
		public int getdeptInfo(ref DataTable result)
		{
			...
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] getdeptInfo failed: {ex.Message}");
					return -1;
				}
			}
		}
		...
		public int getDeptDoctorInfo(string deptName, ref DataTable result)
		{
			...
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] getDeptDoctorInfo failed: {ex.Message}");
					return -1;  
				}
			}
		}
		...
		public int doctorInfoDisplayer(int dID, ref string name, ref string phone, ref string gender, ref float charges_Per_Visit, ref float ReputeIndex, ref int PatientsTreated, ref string qualification, ref string specialization, ref int workE, ref int age)
		{
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] doctorInfoDisplayer failed: {ex.Message}");
					return -1;
				}
			}
		}
		...
		public int getFreeSlots(int dID, int pID, ref DataTable result)
		{
			...
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] getFreeSlots failed: {ex.Message}");
					return -1;  
				}
			}
		}
		...
		public int insertAppointment(int dID, int pID, int freeSlot, ref string mes)
		{
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] insertAppointment failed: {ex.Message}");
					return -1;  
				}
			}
		}
		...
		public int getNotifications(int pid, ref string dName, ref string timings)
		{
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] getNotifications failed: {ex.Message}");
					return -1;  
				}
			}
		}
		...
		public int isFeedbackPending(int pid, ref string dName, ref string timings, ref int aID)
		{
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] isFeedbackPending failed: {ex.Message}");
					return -1; 
				}
			}
		}
		...
		public int givePendingFeedback(int aID)
		{
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] givePendingFeedback failed: {ex.Message}");
					return -1;  
				}
			}
		}
		...
		public int docinfo_DAL(int doctorid, ref DataTable result)
		{
			...
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] docinfo_DAL failed: {ex.Message}");
					return 0;
				}
		...
		public void GetAllpendingappointments_DAL(int doctorid, ref DataTable DT)
		{
			...
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] GetAllpendingappointments_DAL failed: {ex.Message}");
				}
			}
		}
		...
		public int UpdateAppointment_DAL(int Appointmentid)
		{
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] UpdateAppointment_DAL failed: {ex.Message}");
				}
		...
		public int Deleteappointment_DAL(int appointmentid)
		{
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] Deleteappointment_DAL failed: {ex.Message}");
					return -1;
				}
		...
		public int search_patient_DAL(int did, ref DataTable result)
		{
			...
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] search_patient_DAL failed: {ex.Message}");
				}
		...
		public int update_prescription_DAL(int did, int appointid, string disease, string progres, string prescrip)
		{
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] update_prescription_DAL failed: {ex.Message}");
					return 0;
				}
		...
		public int generate_bill_DAL(int docid, ref DataTable result)
		{
			...
			using (SqlConnection con = CreateConnection())
			{
				...
				{
					Console.WriteLine($"[ERROR] generate_bill_DAL failed: {ex.Message}");
					return -1;
				}
        ...
        public int getPHistory(int id, ref DataTable result)
        {
            ...
            using (SqlConnection con = CreateConnection())
            {
				...
				{
					Console.WriteLine($"[ERROR] getPHistory failed: {ex.Message}");
					return -1;
				}
			}
        }
